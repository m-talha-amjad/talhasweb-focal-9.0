  ...
  var InputBindingManager = class {
    ...
    _onValueChanged(event, target) {
      ...
      if (boundElement) {
        ...
        //   boundElement.innerHTML = target.getAttribute("title");
        // }else if( target.hasAttribute("data-custom")){
        //    boundElement.innerHTML = `<span>${target.value}</span><span>${target.getAttribute("data-custom")}</span>`;
        // }else{
      ...
      triggerEvent(document, 'option-change', { variant: document.querySelector('product-variants').selectedVariant } );
    }
  };
...
`;
  ...
  var ProductLoader = class {
    static load(productHandle) {
      ...
      this.loadedProducts[productHandle] = new Promise(async (resolve) => {
        ...
        triggerEvent(document, 'theme-ready', { } );
  ...
  var FacetFilters = class extends DrawerContent {
    connectedCallback() {
      super.connectedCallback();
       console.log(this.rootDelegate);
      this.rootDelegate.on("click", '[data-filter-submit]', this._onFilterChanged.bind(this));
      // this.delegate.on("change", '[name^="filter."]', this._onFilterChanged.bind(this));
      // this.delegate.on("change", 'input[name="sort_by"]', this._onFilterChanged.bind(this));
      this.rootDelegate.on("click", '[data-action="clear-filters"]', this._onFiltersCleared.bind(this));
      this.rootDelegate.on("click", '[data-action="clear-active-filter"]', this._cleareFiltersActive.bind(this));
    ...
    _cleareFiltersActive(event, target){
         event.preventDefault();
         if(target.offsetParent){
             target.offsetParent.querySelectorAll('input').forEach(function(node){ node.checked = false; });
         }
